...
double fwd_spd_factor = 1 / 2.0;
double turn_fwd_spd_factor = 4.0/5.0;
...
void turnLeft() {
  ...
  if (isRealFront) {
    ...
    analogWrite(ENR, turn_fwd_spd_factor*fullSpeed );
    analogWrite(ENL, turn_rev_spd_factor*fullSpeed );
    forwardRightWheels();
    reverseLeftWheels();
  } else {
...
void turnRight() {
  ...
  if (isRealFront) {
    ...
    analogWrite(ENL, fullSpeed * 1 / 1.2);
